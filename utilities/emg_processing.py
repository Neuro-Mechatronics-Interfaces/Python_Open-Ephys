...
def get_file_paths(directory, file_type=None, verbose=False):
    ...
    elif file_type == '.oebin':
        file_paths = list(directory.glob('*'))
        if verbose: print(f"Found {len(file_paths)} sessions")
    elif file_type == '.rhd':
...
def preprocess_emg(emg_data, sample_rate, window_duration=100):
    ...
    # Ensure RMS window size is valid
    rms_window_size = int(window_duration * sample_rate)  # Adjust as needed
    if rms_window_size > filtered_data.shape[1]:
        print(
            f"⚠️ Warning: RMS window size ({rms_window_size}) is larger than signal length ({filtered_data.shape[1]}). Adjusting...")
        rms_window_size = max(1, filtered_data.shape[1] // 10)  # Ensure at least 10 windows

    rms_features = calculate_rms(filtered_data, rms_window_size)
    return rms_features

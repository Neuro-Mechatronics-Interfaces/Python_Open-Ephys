...
import ephys_utilities.ephys_utilities as device_utils
import ephys_utilities.emg_processing as emg_proc
import ephys_utilities.plotting_utilities as plot_utils

def build_ephys_filepaths(root_dir):
    root_dir = Path(root_dir)

    # For ephys binary files, just need the session directories
    session_dir_list =  [str(i) for i in root_dir.iterdir()]
    return session_dir_list
...
class PreProcess:
    ...
    def preprocess_data(self):
        ...
        """
        # Step 1: Get all .oebin file paths in the directory
        cfg = emg_proc.read_config_file(self.cfg)
        file_paths = build_ephys_filepaths(cfg['root_directory'])
        ...
        metrics_filepath = Path(os.path.join(cfg['root_directory'], cfg['metrics_filename']))
        if os.path.isfile(metrics_filepath):
            metrics_file = pd.read_csv(metrics_filepath)
        ...
        for file in file_paths:
            ...
            result, data_present = device_utils.load_file(file)
            #print(result['continuous'].sample_rate)
            ...
            emg_data = result['amplifier_data'].T  # Shape: (num_channels, num_samples)
            time_vector = result['t_amplifier'].T  # Time vector for the data
            ...
            plot_utils.waterfall_plot_old(car_data, [channel_index], time_vector, edges=edges, plot_title=filename,
                                          line_width=0.4, colormap='hsv')
